...
$response = array();
if (isset($_POST['district'])&&isset($_POST['people'])) {
    ...
    try{
        ...
        $result = $db->prepare("SELECT * FROM rentalProperties WHERE district = :district AND avaliability = 'Yes'");
        ...
        if (!empty($result)) {
            ...
            if ($result->rowCount() > 0) {
                ...
                foreach ($result as $row) {
                        ...
                        if ($row['type'] == $people){
                            $posts = array();
                            $posts["supplier"] = $row["supplier"];
                            $posts["type"] = $row["type"];
                            $posts["address"] = $row["address"];
                            $posts["district"] = $row["district"];
                            $posts["features"] = $row["features"];
